                    ...
                    for(String i : changedFiles.unique()){
                        ...
                        if(env.GIT_BRANCH == "dev")  {  
                            ...
                            stage("Deploying ${i} for Prod") {
                                ...
                                dir("${i}"){
                                    bat "mvn deploy -DskipTests -DmuleDeploy -Danypoint.username=%ANYPOINT_CREDENTIALS_USR% -Danypoint.password=%ANYPOINT_CREDENTIALS_PSW% -Danypoint.env=Sandbox -Danypoint.workers=1 -Danypoint.name=${i}-api1-prod -Djar.name=${i}-%APP% -Dmule.artifact=%WORKSPACE%\\${i}\\target\\${i}-%APP%-mule-application.jar"
                                }
                            }
